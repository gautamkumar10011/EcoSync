...
public class DatabaseStorage extends Storage {
    ...
    public <T> List<T> getObjects(Class<T> clazz, Request request) throws StorageException {
        ...
        LOGGER.info("DatabaseStorage(temp) : " + query.toString());
        try {
    ...
    public <T> List<T> getObjects(Class<T> clazz, Request request, String searchKey) throws StorageException {
        if(searchKey == null){
            return getObjects(clazz, request);
        }
        String subQuery = makeSubQuery(clazz, searchKey);
        StringBuilder query = new StringBuilder("SELECT ");
        if (request.getColumns() instanceof Columns.All) {
            query.append('*');
        } else {
        ...
        query.append(formatCondition(request.getCondition()));
        query.append(formatOrder(request.getOrder()));
        String apprentQuery = query.toString();
        String mainQuery = makeMainQuery(clazz, apprentQuery, subQuery);
        LOGGER.info("DatabaseStorage(SearchKey) : " + mainQuery);
        try {
            QueryBuilder builder = QueryBuilder.create(config, dataSource, objectMapper, mainQuery);
            for (Map.Entry<String, Object> variable : getConditionVariables(request.getCondition()).entrySet()) {
                builder.setValue(variable.getKey(), variable.getValue());
            }
            return builder.executeQuery(clazz);
    ...
    private <T> String makeSubQuery(Class<T> clazz, String searchKey) throws StorageException {
        String subQuery = " ( SELECT * FROM " + getStorageName(clazz) +" WHERE name LIKE " + "\"%" + searchKey + "%\") AS subTable " ;
        return subQuery;
    }

    private <T> String makeMainQuery(Class<T> clazz, String query, String subQuery) throws StorageException {
        return query.replace(getStorageName(clazz), subQuery);
    }

    public <T> Count countRecords(Class<T> clazz, Request request, String searchKey) throws StorageException {
        StringBuilder queryPrefix = new StringBuilder("SELECT ");
        queryPrefix.append("COUNT(*) as count ");
        queryPrefix.append(" FROM ").append(getStorageName(clazz));
        queryPrefix.append(formatCondition(request.getCondition()));
        LOGGER.info("DatabaseStorage(CountRecord) : " + queryPrefix.toString());
        String query = queryPrefix.toString();
        if(searchKey != null) {
            /** In case of Ticket searchKey=null **/
            if(query.toLowerCase().contains("where")) {
                query = query + " AND name LIKE " + "\"%" + searchKey + "%\"";
            }else{
        ...
        try {
            ...
            List<Count> result = builder.executeQuery(Count.class);
